...
extern "C" DLLEXPORT bool APIENTRY SKSEPlugin_Query(const SKSE::QueryInterface* a_skse, SKSE::PluginInfo* a_info)
{
	try {
...
#else
		auto path = logger::log_directory() / "ActorCopyLib.log"sv;
		...
		logger::info("ActorCopyLib v{}"sv, MYFP_VERSION_VERSTRING);

		a_info->infoVersion = SKSE::PluginInfo::kVersion;
		a_info->name = "ActorCopyLib";
...
{
	try {
		logger::info("ActorCopyLib loaded");
